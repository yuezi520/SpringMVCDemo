 ...
 * 固定表格头部
 */
var fixed_table_st = window.setTimeout;
window.setTimeout = function(fRef, mDelay) {
    if (typeof fRef == 'function') {
        ...
        return fixed_table_st(f, mDelay);
    }
    return fixed_table_st(fRef, mDelay);
};

function fixedTable(id, obj, _cfg){
	this.id = id;
	this.obj = obj;
	...
	$(window).resize(function(){
		...
		$("#"+_.id + '_div_head').width(_.box.offsetWidth - sw);
		$("#"+_.id + '_tb_header').width(_.obj.offsetWidth)
	});
}

fixedTable.prototype._fixTable = function(_){
...
fixedTable.prototype.buildHead = function(){
	var _ = this;
...
fixedTable.prototype.buildLeft = function(){
	var _ = this;
	if(_.config.cols <= 0){
		return false;
	}
...
fixedTable.prototype.buildTopLeft = function(){
	var _ = this;
